...
def prepareSequences(path_wDir, dict_seqInfo, dict_genomeInfo, int_numCores, int_slopAmount):
    ...
    for seqId, ds in dict_seqInfo.items():
        ...
        pasync(pool, grabBases, args=(path_gtf, int_slopAmount, dict_chrSizes, path_base, path_baseRel, path_baseTight, path_genome, path_cdsFastaOutBase, seqId))
        ...
        pasync(pool, fetchAa,  args=(path_gtf, path_genome, path_cdsFastaOut, path_aaFastaOut, 1))
    pool.close()
    pool.join()
    ...
    sealed = []
        ...
        if not st: break
            ...
            if not sp:
                break
                        r = random.choice(sp)
                        st = [bin_and(r, i) for i in st if bin_compat(i, r, checksB)]
            sp = [s for s in st if not s in sealed]
            if not sp:
                sealed += [r]
                break
...
def getCombinationsFasta(partslist, combis, cds, generegion, minintron, minexon, checkStart=True):
    ...
    for j, c in enumerate(combis):
        ...
        wellOrdered = all(clist[p] <= clist[p+1] for p in range(len(clist)-1))
        goodIntrons = all(clist[(2*p)+1] + minintron <  clist[2*(p+1)] for p in range(len(clist)/2 -1))
...
def aq(alnNc, rgene, ogene, compare, chop):
    ...
    if not changed_nz:
        return False
    if changed_nz:
        if alignmentScore(chopAlignment(alnNc, changed_nz))/len(changed_nz) > 3:
            return False
    return True
...
def chooseThem(path_allAln, tryBlank, seqlookup, seqlookup_rev, path_wDir):
    ...
    sbm = groupSequencesByGeneregion(sequences)
    if len(sbm.keys()) == 1:
        return {sbm.keys()[0]: makeSeq(sbm.keys()[0]+".blank", "")}
